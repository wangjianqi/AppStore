...
2023 年 8 月 15 日正式施行的《生成式人工智能服务管理暂行办法》是中国首部专门针对生成式 AI 的行政法规，对提供和使用生成式 AI 服务的各方都有约束力。

#### 关键条款解读

| 条款 | 内容 | 对 iOS 开发者的影响 |
|------|------|-------------------|
| **算法备案** | 提供生成式 AI 服务需进行算法备案 | 如果 App 内集成 AI 功能，可能需要备案 |
| **安全评估** | 需进行安全评估，确保内容安全 | AI 生成内容需过滤违法信息 |
| **内容标识** | AI 生成内容应进行标识 | App 内 AI 生成的内容需标注"AI 生成" |
| **用户权益** | 用户有权知道内容由 AI 生成 | 需提供 AI 内容的标识和说明 |
| **训练数据** | 训练数据应合法合规 | 使用第三方 AI 服务需关注其合规性 |
| **个人信息** | 不得非法收集使用个人信息 | AI 功能不得过度收集用户数据 |

#### 什么情况下需要算法备案

| 场景 | 是否需要备案 | 说明 |
|------|------------|------|
| App 内调用 OpenAI API | ⚠️ 可能需要 | 取决于是否构成"提供生成式 AI 服务" |
| App 内调用国内大模型 API | ✅ 通常需要 | 模型提供方已备案，App 方可能也需要 |
| App 内使用 Core ML 本地模型 | ❌ 通常不需要 | 本地推理不涉及"服务提供" |
| App 内嵌入 AI 聊天功能 | ✅ 需要 | 构成提供生成式 AI 服务 |
| 仅用 AI 辅助开发（不面向用户） | ❌ 不需要 | 不面向公众提供服务 |
...
| 标识类型 | 说明 | 实现方式 |
|---------|------|---------|
| **显式标识** | 用户可直接看到的标识 | 在 AI 生成内容旁显示"AI 生成"标签 |
| **隐式标识** | 嵌入内容中的水印 | 在 AI 生成图片中嵌入数字水印 |
| **元数据标识** | 在文件元数据中标注 | 在 AI 生成内容的元数据中添加标识 |
...
### 含 AI 功能的 App ICP 备案注意事项

#### 备案流程中的 AI 相关要求

| 步骤 | 常规要求 | AI 额外要求 |
|------|---------|-----------|
| **ICP 备案** | 基本信息填写 | 可能需要额外说明 AI 功能 |
| **App 备案** | 功能描述 | 需描述 AI 功能和使用的模型 |
| **安全评估** | 常规安全评估 | 可能需要算法安全评估 |
| **隐私政策** | 常规隐私条款 | 需说明 AI 功能的数据处理 |

#### 隐私政策中的 AI 条款示例

```
AI 功能说明：
1. 本应用集成了人工智能对话功能，该功能由 [模型提供方] 提供
2. 您与 AI 的对话内容将发送至 [模型提供方] 服务器进行处理
3. 我们不会存储您的对话内容（或：对话内容仅存储在您的设备本地）
4. AI 生成的内容仅供参考，不构成专业建议
5. 您可以选择关闭 AI 功能，关闭后不影响应用的其他功能使用
```
...
| 风险 | 说明 | 影响 |
|------|------|------|
| **数据出境** | 用户数据传输至境外服务器 | 需进行数据出境安全评估 |
| **个人信息保护** | PIPL 对数据出境有严格要求 | 需满足"告知-同意"等前提条件 |
| **合规成本** | 数据出境评估流程复杂 | 增加上线时间和成本 |

#### 合规方案对比

| 方案 | 说明 | 合规性 | 成本 | 推荐度 |
|------|------|--------|------|--------|
| **使用国内大模型 API** | 数据不出境 | ✅ 完全合规 | 低 | ⭐⭐⭐⭐⭐ |
| **后端代理 + 数据脱敏** | 后端转发请求，脱敏后发送 | ⚠️ 部分合规 | 中 | ⭐⭐⭐⭐ |
| **数据出境安全评估** | 完成正式评估流程 | ✅ 合规 | 高 | ⭐⭐⭐ |
| **直接调用海外 API** | 客户端直接调用 | ❌ 高风险 | 低 | ⭐ |

💡 **建议**：面向国内用户的 App，优先使用国内大模型 API（如通义千问、DeepSeek），避免数据出境合规风险。

### 合规检查清单

| 检查项 | 是否完成 | 备注 |
|--------|---------|------|
| AI 功能已在隐私政策中说明 | ☐ | 需描述数据处理方式 |
| AI 生成内容已添加标识 | ☐ | "AI 生成"标签 |
| 用户可选择关闭 AI 功能 | ☐ | 不影响其他功能使用 |
| AI 生成内容已过滤违法信息 | ☐ | 内容安全审核 |
| 已评估是否需要算法备案 | ☐ | 取决于 AI 功能形式 |
| 已评估数据出境合规性 | ☐ | 调用海外 API 需评估 |
| AI 生成内容不构成专业建议 | ☐ | 添加免责声明 |
| 用户数据已最小化收集 | ☐ | 仅收集必要数据 |
